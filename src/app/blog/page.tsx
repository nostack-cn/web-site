          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {sortedPosts.map((post) => (
                <Link
                  key={post.id}
                  href={post.href}
                  className="product-card group flex flex-col rounded-2xl border border-surface-200 bg-white p-6 shadow-sm"
                >
                  {/* 分类标签 */}
                  <div className="mb-4 flex items-center justify-between">
                    <span className={`rounded-full px-3 py-1 text-xs font-medium ${categoryColors[post.category] || 'bg-surface-100 text-ink-600'}`}>
                      {post.category}
                    </span>
                  ...
                  {/* 标题 */}
                  <h2 className="text-lg font-bold text-ink-900 group-hover:text-brand-cyan">
                    {post.title}
                  </h2>
            ...
            {sortedPosts.length === 0 && (
              <div className="py-20 text-center">
                <p className="text-ink-500">暂无文章，敬请期待。</p>
              </div>
            )}
          </div>
        </section>
